/**
 * 分歧地图：把一个问题下的不同立场并排呈现。
 * 页面结构本身就是产品主张——分歧不是要被总结掉的噪声，而是要被并排看见的主语。
 */

import { h, mount, authorityBadge, sourceLink, votes, clear, copyText } from '../dom.js?v=20260913a';
      ...
      h('div', { class: 'evidence-top', style: { marginTop: '10px' } },
        ...
        h('button', {
          ...
          onclick: async (e) => {
            ...
            button.textContent = ok ? '引用已复制' : '复制失败';
            setTimeout(() => { button.textContent = '复制引用'; }, 1600);
          },
        }, '复制引用'))));
  }
}
